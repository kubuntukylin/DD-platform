...
DD Platform 是一个**AI 驱动的低代码微服务开发平台**。它的核心思路是：
...
| 对比 | 直接使用 Claude Code | DD Platform |
|------|---------------------|-------------|
...
**答**：系统使用 SQLite 持久化，数据库文件位于 `%APPDATA%/DD Platform/dd-platform.db`：
- 每次变更后自动保存
